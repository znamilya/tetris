import Tetromino, { Shape } from './Tetromino.js';
import Playfield from './Playfield.js';
import RandomRange from '../../../utils/RandomRange.js';
import Score from './Score.js';

class Game {
  private _randomRange = new RandomRange(0, 6);
  private _score = new Score();
  private _lines = 0;
  private _isOver = false;
  private _currPiece = this.createRandomPiece();
  private _nextPiece = this.createRandomPiece();
  ...
  movePieceDown() {
    ...
    if (!isMoved) {
      ...
      if (
        this._nextPiece.hasCollision(x, y, this._nextPiece, this._playfield)
      ) {
        this._isOver = true;
      }
    }
  ...
  getPlayfieldShapshot(): Playfield {
  ...
  private createRandomPiece() {
    ...
    return Tetromino.create(variants[index] as Shape);
  }
}
